...
server.tool(
  "add_review_comment",
  "Add a review comment to a specific line in a GitHub pull request. Prefer 'line' (the actual file line number) over the deprecated 'position' (an offset within the diff), since 'position' requires re-counting diff hunks and is error-prone.",
  {
    owner: z.string().describe("Repository owner (username or organization)"),
    repo: z.string().describe("Repository name"),
    pr_number: z.number().describe("Pull request number"),
