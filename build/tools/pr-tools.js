...
export class PullRequestTools extends BaseTool {
    ...
    async updatePullRequest(params) {
        return await this.executeOperation('update pull request', async () => {
            ...
            const prData = await this.api.updatePullRequest(params.owner, params.repo, params.pr_number, updateData);
            const updatedFields = Object.keys(updateData).join(', ');
            return this.createSuccessResponse(`Pull request #${prData.number} updated successfully!\n\nUpdated fields: ${updatedFields}\nTitle: ${prData.title}\nURL: ${prData.html_url}`);
        });
    }
    /**
     * オープンなプルリクエスト一覧を取得
