 ...
 * パッチからコメント可能な位置を計算
 */
export function calculateCommentPositions(patch) {
    const positions = [];
    if (!patch) {
        return positions;
    }
 ...
 * ファイル変更情報にコメント位置を追加
 */
export function enrichFileChanges(files) {
 ...
 * プルリクエストの一覧を整形
 */
export function formatPullRequestList(pullRequests, owner, repo) {
 ...
 * プルリクエストのコメントを整形
 */
export function formatComments(issueComments, reviewComments, prNumber) {
 ...
 * ファイル変更情報を整形
 */
export function formatFileChanges(fileChanges, prNumber) {
 ...
 * エラーメッセージを生成
 */
export function createErrorMessage(operation, error) {
